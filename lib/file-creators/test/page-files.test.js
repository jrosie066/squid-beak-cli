...
describe('Page File Creators', () => {
  ...
  beforeEach(() => {
    ...
    componentFileSpy = jest.spyOn(sharedFiles, 'createPageComponentFile').mockImplementation(() => { });
    enhancerFileSpy = jest.spyOn(sharedFiles, 'createPageEnhancerFile').mockImplementation(() => { });
  ...
  describe('#createPageComponent', () => {
    ...
    it('should create the page component file', () => {
      ...
      expect(componentFileSpy).toHaveBeenCalledWith(pageName, expect.stringContaining(root), options);
      expect(enhancerFileSpy).toHaveBeenCalledTimes(1);
      expect(enhancerFileSpy).toHaveBeenCalledWith(pageName, expect.stringContaining(root), options);
