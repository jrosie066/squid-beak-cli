...
describe('GeneratePage', () => {
  ...
  it('create the page if none exist', async () => {
    ...
    expect(mkdirSyncSpy).toHaveBeenCalledWith(root, { recursive: true });
  ...
  it('create the page at a new path if none exist', async () => {
    existsSyncSpy.mockReturnValue(false);
    const newPath = 'new/path';
    options.newPath = newPath;
    await initiatePage(pageName, options);
    expect(mkdirSyncSpy).toHaveBeenCalledTimes(1);
    expect(mkdirSyncSpy).toHaveBeenCalledWith(expect.stringContaining(newPath), { recursive: true });
    expect(pageMainSpy).toHaveBeenCalledTimes(1);
    expect(pageMainSpy).toHaveBeenCalledWith('Test', expect.stringContaining(newPath), options);
    expect(pageComponentSpy).toHaveBeenCalledTimes(1);
    expect(pageComponentSpy).toHaveBeenCalledWith('Test', expect.stringContaining(newPath), options);
    expect(pageWrappereSpy).toHaveBeenCalledTimes(1);
    expect(pageWrappereSpy).toHaveBeenCalledWith('Test', expect.stringContaining(newPath));
    expect(updateRouteSpy).toHaveBeenCalledTimes(1);
    expect(updateRouteSpy).toHaveBeenCalledWith('Test');
    expect(lintRoller).toHaveBeenCalledTimes(1);
    expect(lintRoller).toHaveBeenCalledWith(`${newPath}/Test`);
  });
  ...
  it('should handle lint roller error', async () => {
    ...
    expect(error).toEqual('Clean up done with some errors');
  });
});