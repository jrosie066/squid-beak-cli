...
const installAlloyEslint = async (projectName) => {
  const stepName = 'Installing Alloy Eslint';
  const command = 'npm install --save-dev babel-eslint eslint-plugin-react eslint-config-alloy';
  await runStepCommand(projectName, stepName, command);
};
...
const lintProject = async (projectName) => {
  ...
  let command = 'FOLDER=src npm run lint';
  await runStepCommand(projectName, stepName, command);
};
